Feature: Courses
    ...
    | Beer Taster | 3     | Creative and design           | 1.0     | Approved for delivery |
	...
	Given I have an http client
    When I GET the following url: /api/courses/search?levels=1&levels=7&levels=3&filter=ActiveAvailable
    Then an http status code of 200 is returned
    And the following valid courses are returned
    | title       | level | route                         | version | status                |
    | Brewer      | 1     | Engineering and manufacturing | 1.0     | Approved for delivery |
